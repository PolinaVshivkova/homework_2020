...
from films.ui_table_form_ import Ui_MainWindow
from films.ui_create_form import Ui_Dialog
from films.ui_edit_form import Ui_Dialog_2
...
class EditForm(QDialog, Ui_Dialog_2):
    ...
    def ok_action(self):
        ...
        try:
            ...
            buffer = [self.lineEdit.text(), self.lineEdit_2.text(), self.lineEdit_3.text(), genre, self.lineEdit_5.text()]
            self.accept()
...
class CreateForm(QDialog, Ui_Dialog):
    ...
    def ok_action(self):
        try:
            ...
            if not flag_id:
                cursor.execute(f"""INSERT INTO films (title, year, genre, duration) VALUES ("{title}", {year}, {genre}, {duration})""")
            elif flag_id:
                cursor.execute(f"""INSERT INTO films (id, title, year, genre, duration) VALUES ({id_}, {self.lineEdit_2.text()}, {year}, {genre}, {duration})""")
